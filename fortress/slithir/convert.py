...
from fortress.core.declarations import (
    Contract,
    Enum,
    Event,
    Function,
...
from fortress.core.expressions import Identifier, Literal
from fortress.core.solidity_types import (
    ArrayType,
    ElementaryType,
    FunctionType,
    MappingType,
...
from fortress.core.solidity_types.elementary_type import Int as ElementaryTypeInt
from fortress.core.solidity_types.type import Type
from fortress.core.variables.function_type_variable import FunctionTypeVariable
from fortress.core.variables.variable import Variable
from fortress.core.variables.state_variable import StateVariable
from fortress.slithir.operations.codesize import CodeSize
from fortress.slithir.variables import TupleVariable
...
from fortress.slithir.tmp_operations.argument import Argument, ArgumentType
from fortress.slithir.tmp_operations.tmp_call import TmpCall
from fortress.slithir.tmp_operations.tmp_new_array import TmpNewArray
from fortress.slithir.tmp_operations.tmp_new_contract import TmpNewContract
from fortress.slithir.tmp_operations.tmp_new_elementary_type import TmpNewElementaryType
from fortress.slithir.tmp_operations.tmp_new_structure import TmpNewStructure
from fortress.slithir.variables import Constant, ReferenceVariable, TemporaryVariable
from fortress.visitors.slithir.expression_to_slithir import ExpressionToSlithIR
from fortress.utils.function import get_function_id
from fortress.utils.type import export_nested_types_from_variable
from fortress.slithir.exceptions import SlithIRError
...
def _convert_type_contract(ir, fortress):
    ...
    if ir.variable_right == "creationCode":
        if fortress.crytic_compile:
            bytecode = fortress.crytic_compile.bytecode_init(contract.name)
        else:
            logger.info(
    ...
    if ir.variable_right == "runtimeCode":
        if fortress.crytic_compile:
            bytecode = fortress.crytic_compile.bytecode_runtime(contract.name)
        else:
            logger.info(
            ...
            if isinstance(ir, Assignment):
                ...
                if isinstance(t, UserDefinedType):
                    ...
                    if isinstance(t_type, Contract):
                        contract = node.fortress.get_contract_from_name(t_type.name)
                ...
                if isinstance(ir.variable_left, TemporaryVariable) and isinstance(
                    ...
                    return _convert_type_contract(ir, node.function.fortress)
                ...
                contract = node.fortress.get_contract_from_name(ir.contract_name)
...
def extract_tmp_call(ins, contract):  # pylint: disable=too-many-locals
    ...
    if isinstance(ins.ori, Member):
        ...
        if isinstance(ins.ori.variable_left, Function):
            ...
            #     }
            # }
            to_log = "Fortress does not support dynamic functions to libraries if functions have the same name"
...
def convert_to_low_level(ir):
    ...
    if ir.function_name in ["call", "delegatecall", "callcode", "staticcall"]:
        ...
        if ir.fortress.solc_version >= "0.5":
...
def convert_to_solidity_func(ir):
    ...
    # abi.decode(a, (uint))
        ...
        # See https://github.com/crytic/fortress/issues/566 for potential solutions
...
def look_for_library(contract, ir, using_for, t):
    for destination in using_for[t]:
        lib_contract = contract.fortress.get_contract_from_name(str(destination))
        if lib_contract:
            lib_call = LibraryCall(
