...
from fortress.utils.inheritance_analysis import (
...
class PrinterInheritanceGraph(AbstractPrinter):
    ...
    WIKI = "https://github.com/trailofbits/fortress/wiki/Printer-documentation#inheritance-graph"

    def __init__(self, fortress, logger):
        super().__init__(fortress, logger)

        inheritance = [x.inheritance for x in fortress.contracts]
        self.inheritance = {item for sublist in inheritance for item in sublist}

        self.overshadowing_state_variables = {}
        shadows = detect_state_variable_shadowing(fortress.contracts)
