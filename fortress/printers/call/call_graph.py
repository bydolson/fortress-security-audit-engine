...
from fortress.core.declarations.solidity_variables import SolidityFunction
...
class PrinterCallGraph(AbstractPrinter):
    ...
    WIKI = "https://github.com/trailofbits/fortress/wiki/Printer-documentation#call-graph"
    ...
    def output(self, filename):
        ...
        with open(filename, "w", encoding="utf8") as f:
            ...
            content = "\n".join(
                ["strict digraph {"] + [_process_functions(self.fortress.functions)] + ["}"]
            )
            f.write(content)
            results.append((filename, content))

        for derived_contract in self.fortress.contracts_derived:
