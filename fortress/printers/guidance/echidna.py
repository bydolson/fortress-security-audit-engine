...
from fortress.analyses.data_dependency.data_dependency import is_dependent
...
from fortress.core.expressions import NewContract
...
from fortress.slithir.operations.binary import Binary
from fortress.slithir.variables import Constant
...
def _extract_payable(fortress: FortressCore) -> Dict[str, List[str]]:
...
def _extract_solidity_variable_usage(
    fortress: FortressCore, sol_var: SolidityVariable
) -> Dict[str, List[str]]:
    ret: Dict[str, List[str]] = {}
    for contract in fortress.contracts:
...
def _is_constant(f: Function) -> bool:  # pylint: disable=too-many-branches
    ...
    if f.view or f.pure:
        if f.contract.fortress.crytic_compile and f.contract.fortress.crytic_compile.compiler_version:
            if not f.contract.fortress.crytic_compile.compiler_version.version.startswith("0.4"):
                return True
    if f.payable:
        return False
    ...
    for ir in f.all_slithir_operations():
        ...
        if isinstance(ir, HighLevelCall):
            if isinstance(ir.function, Variable) or ir.function.view or ir.function.pure:
                ...
                if (
                    f.contract.fortress.crytic_compile
                    and f.contract.fortress.crytic_compile.compiler_version
                ):
                    if f.contract.fortress.crytic_compile.compiler_version.version.startswith("0.4"):
                        return False
            else:
                return False
...
def _extract_constant_functions(fortress: FortressCore) -> Dict[str, List[str]]:
    ret: Dict[str, List[str]] = {}
    for contract in fortress.contracts:
...
def _have_external_calls(fortress: FortressCore) -> Dict[str, List[str]]:
    """
...
def _use_balance(fortress: FortressCore) -> Dict[str, List[str]]:
    """
    Detect the functions with external calls
    :param fortress:
    :return:
...
def _call_a_parameter(fortress: FortressCore) -> Dict[str, List[Dict]]:
    """
    Detect the functions with external calls
    :param fortress:
    :return:
    ...
    for contract in fortress.contracts:  # pylint: disable=too-many-nested-blocks
        for function in contract.functions_entry_points:
            for ir in function.all_slithir_operations():
                if isinstance(ir, HighLevelCall):
...
class Echidna(AbstractPrinter):
    ...
    WIKI = "https://github.com/trailofbits/fortress/wiki/Printer-documentation#echidna"
    ...
    def output(self, filename):  # pylint: disable=too-many-locals
        ...
        payable = _extract_payable(self.fortress)
        timestamp = _extract_solidity_variable_usage(
            self.fortress, SolidityVariableComposed("block.timestamp")
        )
        block_number = _extract_solidity_variable_usage(
            self.fortress, SolidityVariableComposed("block.number")
        )
        msg_sender = _extract_solidity_variable_usage(
            self.fortress, SolidityVariableComposed("msg.sender")
        )
        msg_gas = _extract_solidity_variable_usage(
            self.fortress, SolidityVariableComposed("msg.gas")
        )
        assert_usage = _extract_assert(self.fortress)
        cst_functions = _extract_constant_functions(self.fortress)
        (cst_used, cst_used_in_binary) = _extract_constants(self.fortress)

        functions_relations = _extract_function_relations(self.fortress)
        ...
        external_calls = _have_external_calls(self.fortress)

        call_parameters = _call_a_parameter(self.fortress)

        use_balance = _use_balance(self.fortress)
