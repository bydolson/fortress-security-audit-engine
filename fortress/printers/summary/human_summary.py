...
from fortress.core.declarations import SolidityFunction, Function
...
from fortress.utils.code_complexity import compute_cyclomatic_complexity
from fortress.utils.colors import green, red, yellow
from fortress.utils.myprettytable import MyPrettyTable
from fortress.utils.standard_libraries import is_standard_library
from fortress.core.cfg.node import NodeType
from fortress.utils.tests_pattern import is_test_file
...
class PrinterHumanSummary(AbstractPrinter):
    ...
    WIKI = "https://github.com/trailofbits/fortress/wiki/Printer-documentation#human-summary"
    ...
    def _get_detectors_result(self) -> Tuple[List[Dict], int, int, int, int, int]:
        ...
        checks_optimization = self.fortress.detectors_optimization
        checks_informational = self.fortress.detectors_informational
        checks_low = self.fortress.detectors_low
        checks_medium = self.fortress.detectors_medium
        checks_high = self.fortress.detectors_high
    ...
    def _lines_number(self):
        if not self.fortress.source_code:
            return None
        total_dep_lines = 0
        total_lines = 0
        total_tests_lines = 0

        for filename, source_code in self.fortress.source_code.items():
            lines = len(source_code.splitlines())
            is_dep = False
            if self.fortress.crytic_compile:
                is_dep = self.fortress.crytic_compile.is_dependency(filename)
            if is_dep:
    ...
    def _compilation_type(self):
        ...
        return f"Compiled with {str(self.fortress.crytic_compile.type)}\n"

    def _number_contracts(self):
        if self.fortress.crytic_compile is None:
            return len(self.fortress.contracts), 0, 0
        contracts = [c for c in self.fortress.contracts if not c.is_top_level]
    ...
    def _get_features(self, contract):  # pylint: disable=too-many-branches
        ...
        for pragma in self.fortress.pragma_directives:
            if (
