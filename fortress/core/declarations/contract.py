...
from fortress.core.children.child_fortress import ChildFortress
from fortress.core.solidity_types.type import Type
from fortress.core.source_mapping.source_mapping import SourceMapping

from fortress.core.declarations.function import Function, FunctionType
from fortress.utils.erc import (
    ERC20_signatures,
    ERC165_signatures,
    ERC223_signatures,
    ERC721_signatures,
...
from fortress.utils.tests_pattern import is_test_contract

# pylint: disable=too-many-lines,too-many-instance-attributes,import-outside-toplevel,too-many-nested-blocks
if TYPE_CHECKING:
    from fortress.utils.type_helpers import LibraryCallType, HighLevelCallType, InternalCallType
    from fortress.core.declarations import Enum, Event, Modifier
    from fortress.core.declarations import Structure
...
class Contract(ChildFortress, SourceMapping):  # pylint: disable=too-many-public-methods
    """
    Contract class
    """
    ...
    def derived_contracts(self) -> List["Contract"]:
        ...
        """
        candidates = self.fortress.contracts
    ...
    def is_from_dependency(self) -> bool:
        if self.fortress.crytic_compile is None:
            return False
        return self.fortress.crytic_compile.is_dependency(self.source_mapping["filename_absolute"])
    ...
    def is_truffle_migration(self) -> bool:
        ...
        :return:
        """
        if self.fortress.crytic_compile:
            if self.fortress.crytic_compile.platform == PlatformType.TRUFFLE:
    ...
    def is_upgradeable(self) -> bool:
        if self._is_upgradeable is None:
            ...
            initializable = self.fortress.get_contract_from_name("Initializable")
            if initializable:
    ...
    def is_upgradeable_proxy(self) -> bool:
        ...
        from fortress.slithir.operations import LowLevelCall
    ...
    def is_incorrectly_constructed(self) -> bool:
        """
        Return true if there was an internal Fortress's issue when analyzing the contract
        :return:
        """
        ...
        if self.state_variables:
            for (idx, variable_candidate) in enumerate(self.state_variables):
                if variable_candidate.expression and not variable_candidate.is_constant:
                    ...
                    constructor_variable.set_offset(self.source_mapping, self.fortress)
    ...
    def _create_node(self, func: Function, counter: int, variable: "Variable"):
        ...
        node.set_offset(variable.source_mapping, self.fortress)
        ...
        expression.set_offset(variable.source_mapping, self.fortress)
    ...
    def convert_expression_to_slithir_ssa(self):
        ...
        :return:
        """
        from fortress.slithir.variables import StateIRVariable
