...
from fortress.core.context.context import Context
...
class SourceMapping(Context):
    ...
    @staticmethod
    def _compute_line(fortress, filename, start: int, length: int) -> Tuple[List[int], int, int]:
        """
        ...
        """
        start_line, starting_column = fortress.crytic_compile.get_line_from_offset(filename, start)
        end_line, ending_column = fortress.crytic_compile.get_line_from_offset(
            filename, start + length
        )
        return list(range(start_line, end_line + 1)), starting_column, ending_column

    def _convert_source_mapping(self, offset: str, fortress):  # pylint: disable=too-many-locals
        """
        ...
        """
        sourceUnits = fortress.source_units
        ...
        if fortress.crytic_compile:
            filenames = fortress.crytic_compile.filename_lookup(filename_used)
            filename_absolute = filenames.absolute
            filename_relative = filenames.relative
            filename_short = filenames.short

            is_dependency = fortress.crytic_compile.is_dependency(filename_absolute)

            if (
                filename_absolute in fortress.source_code
                or filename_absolute in fortress.crytic_compile.src_content
            ):
                filename = filename_absolute
            elif filename_relative in fortress.source_code:
                filename = filename_relative
            elif filename_short in fortress.source_code:
        ...
        if fortress.crytic_compile:
            (lines, starting_column, ending_column) = self._compute_line(fortress, filename, s, l)
        else:
    ...
    def set_offset(self, offset: Union[Dict, str], fortress):
        if isinstance(offset, dict):
            self._source_mapping = offset
        else:
            self._source_mapping = self._convert_source_mapping(offset, fortress)
