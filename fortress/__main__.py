...
from fortress.detectors import all_detectors
from fortress.detectors.abstract_detector import AbstractDetector, DetectorClassification
from fortress.printers import all_printers
from fortress.printers.abstract_printer import AbstractPrinter
from fortress.fortress import Fortress
from fortress.utils.output import output_to_json, output_to_zip, ZIP_TYPES_ACCEPTED
from fortress.utils.output_capture import StandardOutputCapture
from fortress.utils.colors import red, blue, set_colorization_enabled
from fortress.utils.command_line import (
    output_detectors,
...
def process_single(target, args, detector_classes, printer_classes):
    """
    The core high-level code for running Fortress static analysis.
    ...
    if args.legacy_ast:
        ast = "--ast-json"
    fortress = Fortress(target, ast_format=ast, **vars(args))

    return _process(fortress, detector_classes, printer_classes)
...
def process_all(target, args, detector_classes, printer_classes):
    ...
    for compilation in compilations:
        ...
        fortress_instances.append(fortress)
...
def _process(fortress, detector_classes, printer_classes):
    for detector_cls in detector_classes:
        fortress.register_detector(detector_cls)

    for printer_cls in printer_classes:
        fortress.register_printer(printer_cls)

    analyzed_contracts_count = len(fortress.contracts)
    ...
    if not printer_classes:
        ...
        printer_results = fortress.run_printers()
        printer_results = [x for x in printer_results if x]  # remove empty results
        results_printers.extend(printer_results)

    return fortress, results_detectors, results_printers, analyzed_contracts_count
...
def get_detectors_and_printers():
    ...
    # Handle plugins!
    for entry_point in iter_entry_points(group="fortress_analyzer.plugin", name=None):
...
def parse_args(detector_classes, printer_classes):
    parser = argparse.ArgumentParser(
        description="Fortress. For usage information, see https://github.com/crytic/fortress/wiki/Usage",
        usage="fortress.py contract.sol [flag]",
    )
    ...
    parser.add_argument(
        ...
        version=require("fortress-analyzer")[0].version,
        action="version",
    )
    ...
    group_misc.add_argument(
        "--triage-mode",
        help="Run triage mode (save results in fortress.db.json)",
        action="store_true",
        dest="triage_mode",
        default=False,
    )
...
def main_impl(all_detector_classes, all_printer_classes):
    ...
    """
    # Set logger of Fortress to info, to catch warnings related to the arg parsing
    ...
    try:
        ...
        if args.solc_ast or (filename.endswith(".json") and not is_supported(filename)):
            ...
            if args.splitted:
                ...
                fortress_instances.append(fortress_instance)
            else:
                for filename in filenames:
                    (
                        fortress_instance,
        ...
        if outputting_json or outputting_zip:
            ...
            if "compilations" in args.json_types:
                compilation_results = []
                for fortress_instance in fortress_instances:
                    compilation_results.append(
                        generate_standard_export(fortress_instance.crytic_compile)
                    )
    ...
    except FortressException as fortress_exception:
        output_error = str(fortress_exception)
        traceback.print_exc()
        logging.error(red("Error:"))
        logging.error(red(output_error))
        logging.error("Please report an issue to https://github.com/crytic/fortress/issues")
