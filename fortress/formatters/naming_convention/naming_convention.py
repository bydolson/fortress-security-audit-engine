import re
import logging
from fortress.slithir.operations import (
    Send,
    Transfer,
    OperationWithLValue,
    HighLevelCall,
...
from fortress.core.declarations import Modifier
from fortress.core.solidity_types import UserDefinedType, MappingType
from fortress.core.declarations import Enum, Contract, Structure, Function
from fortress.core.solidity_types.elementary_type import ElementaryTypeName
from fortress.core.variables.local_variable import LocalVariable
from fortress.formatters.exceptions import FormatError, FormatImpossible
...
def custom_format(fortress, result):
    ...
    for element in elements:
        ...
        _patch(fortress, result, element, target)
...
def _name_already_use(fortress, name):
    # Do not convert to a name used somewhere else
    if not KEY in fortress.context:
        all_names = set()
        ...
        fortress.context[KEY] = all_names
    return name in fortress.context[KEY]


def _convert_CapWords(original_name, fortress):
    ...
    if _name_already_use(fortress, name):
...
def _convert_mixedCase(original_name, fortress):
    name = original_name
...
def _convert_UPPER_CASE_WITH_UNDERSCORES(name, fortress):
    if _name_already_use(fortress, name.upper()):
...
def _get_from_contract(fortress, element, name, getter):
    contract_name = element["type_specific_fields"]["parent"]["name"]
    contract = fortress.get_contract_from_name(contract_name)
...
def _patch(fortress, result, element, _target):
    if _target == "contract":
        target = fortress.get_contract_from_name(element["name"])

    elif _target == "structure":
        target = _get_from_contract(fortress, element, element["name"], "get_structure_from_name")

    elif _target == "event":
        target = _get_from_contract(fortress, element, element["name"], "get_event_from_name")
        ...
        if element["name"] != element["type_specific_fields"]["parent"]["name"]:
            ...
            target = _get_from_contract(
                fortress, element, function_sig, "get_function_from_signature"
            )

    elif _target == "modifier":
        modifier_sig = element["type_specific_fields"]["signature"]
        target = _get_from_contract(fortress, element, modifier_sig, "get_modifier_from_signature")
        ...
        # State variable
            ...
            target = _get_from_contract(
                fortress, element, element["name"], "get_state_variable_from_name"
            )

    elif _target == "enum":
        target = _get_from_contract(
            fortress, element, element["name"], "get_enum_from_canonical_name"
        )

    else:
        raise FormatError("Unknown naming convention! " + _target)

    _explore(fortress, result, target, conventions[element["additional_fields"]["convention"]])
...
def _explore_type(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches
    fortress, result, target, convert, custom_type, filename_source_code, start, end
):
    if isinstance(custom_type, UserDefinedType):
        ...
        if isinstance(custom_type.type, (Enum, Contract)):
            ...
            _explore_variables_declaration(
                fortress, custom_type.type.elems.values(), result, target, convert
            )
    ...
    if isinstance(custom_type, MappingType):
        ...
        if isinstance(custom_type.type_to, (UserDefinedType, MappingType)) or target in [
            ...
            full_txt_end = end
            full_txt = fortress.source_code[filename_source_code].encode("utf8")[
...
def _explore_variables_declaration(  # pylint: disable=too-many-arguments,too-many-locals,too-many-nested-blocks
    fortress, variables, result, target, convert, patch_comment=False
):
            ...
            if patch_comment and isinstance(variable, LocalVariable):
                if "lines" in variable.source_mapping and variable.source_mapping["lines"]:
                    ...
                    if variable in func.parameters:
                        ...
                        potential_comments = fortress.source_code[filename_source_code].encode(
                            "utf8"
                        )
...
def _explore_structures_declaration(fortress, structures, result, target, convert):
    for st in structures:
        # Explore the variable declared within the structure (VariableStructure)
        _explore_variables_declaration(fortress, st.elems.values(), result, target, convert)
...
def _explore_events_declaration(fortress, events, result, target, convert):
    for event in events:
        ...
        _explore_variables_declaration(fortress, event.elems, result, target, convert)
...
def _explore_functions(fortress, functions, result, target, convert):
    for function in functions:
        _explore_variables_declaration(fortress, function.variables, result, target, convert, True)
        _explore_irs(fortress, function.all_slithir_operations(), result, target, convert)
...
def _explore_contract(fortress, contract, result, target, convert):
    _explore_variables_declaration(fortress, contract.state_variables, result, target, convert)
    _explore_structures_declaration(fortress, contract.structures, result, target, convert)
    _explore_functions(fortress, contract.functions_and_modifiers, result, target, convert)
...
def _explore(fortress, result, target, convert):
    for contract in fortress.contracts_derived:
        _explore_contract(fortress, contract, result, target, convert)
