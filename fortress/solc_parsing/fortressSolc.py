...
from fortress.core.declarations.pragma_directive import Pragma
from fortress.core.declarations.import_directive import Import
from fortress.analyses.data_dependency.data_dependency import compute_dependency

logging.basicConfig()
logger = logging.getLogger("FortressSolcParsing")
logger.setLevel(logging.INFO)


class FortressSolc:
    # pylint: disable=no-self-use,too-many-instance-attributes
    def __init__(self, filename: str, core: FortressCore):
        super().__init__()
    ...
    def parse_contracts_from_loaded_json(
        ...
        for contract_data in data_loaded[self.get_children()]:
            ...
            if contract_data[self.get_key()] == "ContractDefinition":
                ...
                fake_contract_data = {
                    "name": f"FortressInternalTopLevelContract{self._top_level_contracts_counter}",
                    "id": -1000
    ...
    def parse_contracts(self):  # pylint: disable=too-many-statements,too-many-branches
        ...
        for contract in self._underlying_contract_to_parser:
            if (
                contract.name.startswith("FortressInternalTopLevelContract")
...
Please rename it, this name is reserved for Fortress's internals"""
    ...
    def analyze_contracts(self):  # pylint: disable=too-many-statements,too-many-branches
        if not self._parsed:
            raise FortressException("Parse the contract before running analyses")
