...
from fortress.core.declarations import Modifier, Structure, Event
from fortress.core.declarations.contract import Contract
from fortress.core.declarations.enum import Enum
from fortress.core.declarations.function import Function
from fortress.core.variables.state_variable import StateVariable
from fortress.solc_parsing.declarations.event import EventSolc
from fortress.solc_parsing.declarations.function import FunctionSolc
from fortress.solc_parsing.declarations.modifier import ModifierSolc
from fortress.solc_parsing.declarations.structure import StructureSolc
from fortress.solc_parsing.exceptions import ParsingError, VariableNotFound
from fortress.solc_parsing.solidity_types.type_parsing import parse_type
from fortress.solc_parsing.variables.state_variable import StateVariableSolc

LOGGER = logging.getLogger("ContractSolcParsing")

if TYPE_CHECKING:
    from fortress.solc_parsing.fortressSolc import FortressSolc
    from fortress.core.fortress_core import FortressCore

# pylint: disable=too-many-instance-attributes,import-outside-toplevel,too-many-nested-blocks,too-many-public-methods


class ContractSolc:
    def __init__(self, fortress_parser: "FortressSolc", contract: Contract, data):
        # assert fortressSolc.solc_version.startswith('0.4')

        self._contract = contract
        self._contract.set_fortress(fortress_parser.core)
        self._fortress_parser = fortress_parser
        self._data = data
    ...
    @property
    def fortress(self) -> "FortressCore":
        return self._contract.fortress

    @property
    def fortress_parser(self) -> "FortressSolc":
        return self._fortress_parser
    ...
    def get_key(self) -> str:
        return self._fortress_parser.get_key()
    ...
    def is_compact_ast(self) -> bool:
        return self._fortress_parser.is_compact_ast
    ...
    def _parse_struct(self, struct: Dict):
        ...
        st.set_offset(struct["src"], self._contract.fortress)
    ...
    def parse_state_variables(self):
        ...
        for varNotParsed in self._variablesNotParsed:
            var = StateVariable()
            var.set_offset(varNotParsed["src"], self._contract.fortress)
    ...
    def _parse_modifier(self, modifier_data: Dict):
        modif = Modifier()
        modif.set_offset(modifier_data["src"], self._contract.fortress)
        ...
        self._contract.fortress.add_modifier(modif)
        self._modifiers_no_params.append(modif_parser)
        self._modifiers_parser.append(modif_parser)

        self._fortress_parser.add_functions_parser(modif_parser)
    ...
    def _parse_function(self, function_data: Dict):
        func = Function()
        func.set_offset(function_data["src"], self._contract.fortress)
        ...
        self._contract.fortress.add_function(func)
        self._functions_no_params.append(func_parser)
        self._functions_parser.append(func_parser)

        self._fortress_parser.add_functions_parser(func_parser)
    ...
    def log_incorrect_parsing(self, error):
        if self._contract.fortress.disallow_partial:
    ...
    def _analyze_params_elements(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        try:
            for father in self._contract.inheritance:
                father_parser = self._fortress_parser.underlying_contract_to_parser[father]
                for element_parser in getter(father_parser):
                    ...
                    elem.set_offset(
                        element_parser.function_not_parsed["src"], self._contract.fortress,
                    )

                    elem_parser = Cls_parser(elem, element_parser.function_not_parsed, self,)
                    elem_parser.analyze_params()
                    if isinstance(elem, Modifier):
                        self._contract.fortress.add_modifier(elem)
                    else:
                        self._contract.fortress.add_function(elem)

                    self._fortress_parser.add_functions_parser(elem_parser)
    ...
    def _analyze_enum(self, enum):
        ...
        new_enum.set_offset(enum["src"], self._contract.fortress)
    ...
    def analyze_events(self):
        try:
            ...
            for event_to_parse in self._eventsNotParsed:
                ...
                event.set_offset(event_to_parse["src"], self._contract.fortress)
