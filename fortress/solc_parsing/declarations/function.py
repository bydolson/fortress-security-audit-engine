...
from fortress.core.cfg.node import NodeType, link_nodes, insert_node, Node
from fortress.core.declarations.contract import Contract
from fortress.core.declarations.function import (
    Function,
    ModifierStatements,
    FunctionType,
)

from fortress.core.expressions import AssignmentOperation
from fortress.core.variables.local_variable import LocalVariable
from fortress.core.variables.local_variable_init_from_tuple import LocalVariableInitFromTuple

from fortress.solc_parsing.cfg.node import NodeSolc
from fortress.solc_parsing.expressions.expression_parsing import parse_expression
from fortress.solc_parsing.variables.local_variable import LocalVariableSolc
from fortress.solc_parsing.variables.local_variable_init_from_tuple import (
    LocalVariableInitFromTupleSolc,
)
from fortress.solc_parsing.variables.variable_declaration import MultipleVariablesDeclaration
from fortress.solc_parsing.yul.parse_yul import YulBlock
from fortress.utils.expression_manipulations import SplitTernaryExpression
from fortress.visitors.expression.export_values import ExportValues
from fortress.visitors.expression.has_conditional import HasConditional
from fortress.solc_parsing.exceptions import ParsingError
from fortress.core.source_mapping.source_mapping import SourceMapping

if TYPE_CHECKING:
    from fortress.core.expressions.expression import Expression
...
class FunctionSolc:
    ...
    def __init__(
        ...
        self._fortress_parser: "FortressSolc" = contract_parser.fortress_parser
    ...
    def fortress(self) -> "FortressCore":
        return self._function.fortress
    ...
    def _parse_try_catch(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        ...
        if externalCall is None:
            raise ParsingError("Try/Catch not correctly parsed by Fortress %s" % statement)
    ...
    def _parse_catch(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        ...
        if block is None:
            raise ParsingError("Catch not correctly parsed by Fortress %s" % statement)
    ...
    def _parse_variable_definition(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        try:
            ...
            local_var.set_offset(statement["src"], self._function.fortress)
    ...
    def _parse_statement(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        ...
        if name == "IfStatement":
            ...
            if "AST" in statement and not self.fortress.skip_assembly:
    ...
    def _add_param(self, param: Dict) -> LocalVariableSolc:
        ...
        local_var = LocalVariable()
        local_var.set_function(self._function)
        local_var.set_offset(param["src"], self._function.fortress)
    ...
    def _parse_params(self, params: Dict):
        ...
        self.parameters_src.set_offset(params["src"], self._function.fortress)
    ...
    def _parse_returns(self, returns: Dict):
        ...
        self.returns_src.set_offset(returns["src"], self._function.fortress)
