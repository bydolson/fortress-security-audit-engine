...
from fortress.core.cfg.node import NodeType, Node, link_nodes
...
from fortress.visitors.expression.find_calls import FindCalls
from fortress.visitors.expression.read_var import ReadVar
from fortress.visitors.expression.write_var import WriteVar
    ...
    def analyze_expressions(self):
        ...
        if self._node.expression:
            if self._node.type == NodeType.VARIABLE:
                ...
                _expression.set_offset(self._node.expression.source_mapping, self._node.fortress)
...
class YulLocalVariable:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, var: LocalVariable, root: YulScope, ast: Dict):
        ...
        var.set_offset(ast["src"], root.fortress)
...
class YulFunction(YulScope):
    ...
    def __init__(self, func: Function, root: YulScope, ast: Dict):
        ...
        func.set_offset(ast["src"], root.fortress)
    ...
    @property
    def fortress(self) -> FortressCore:
        return self._entrypoint.underlying_node.fortress
    ...
    def new_node(self, node_type, src) -> YulNode:
        if self._function:
            ...
            raise FortressException("standalone yul objects are not supported yet")
...
def convert_yul_function_definition(root: YulScope, parent: YulNode, ast: Dict) -> YulNode:
    ...
    root.fortress.add_function(func)
...
def parse_yul_function_call(root: YulScope, node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    if not isinstance(ident, Identifier):
        raise FortressException("expected identifier from parsing function name")
    ...
    raise FortressException(f"unexpected function call target type {str(type(ident.value))}")
...
def parse_yul_unsupported(_root: YulScope, _node: YulNode, ast: Dict) -> Optional[Expression]:
    raise FortressException(f"no parser available for {ast['nodeType']} {json.dumps(ast, indent=2)}")
...
def parse_yul(root: YulScope, node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    if op:
        op.set_offset(ast["src"], root.fortress)
    return op
