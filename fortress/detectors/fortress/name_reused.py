...
def _find_missing_inheritance(fortress):
    """
    ...
    :return:
    """
    missings = fortress.contracts_with_missing_inheritance
...
class NameReused(AbstractDetector):
    ...
    WIKI = "https://github.com/crytic/fortress/wiki/Detector-Documentation#name-reused"
...
"""
    ...
    def _detect(self):  # pylint: disable=too-many-locals,too-many-branches
        ...
        names_reused = self.fortress.contract_name_collisions
        ...
        most_base_with_missing_inheritance = _find_missing_inheritance(self.fortress)

        for b in most_base_with_missing_inheritance:
            info = [b, " inherits from a contract for which the name is reused.\n"]
            if b.inheritance:
                info += ["\t- Fortress could not determine which contract has a duplicate name:\n"]
