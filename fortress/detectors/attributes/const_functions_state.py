...
from fortress.formatters.attributes.const_functions import custom_format
...
class ConstantFunctionsState(AbstractDetector):
    ...
    ARGUMENT = "constant-function-state"  # run the detector with fortress.py --ARGUMENT
    HELP = "Constant functions changing the state"  # help information
    IMPACT = DetectorClassification.MEDIUM
    CONFIDENCE = DetectorClassification.MEDIUM

    WIKI = "https://github.com/crytic/fortress/wiki/Detector-Documentation#constant-functions-changing-the-state"
...
All the calls to `get` revert, breaking Bob's smart contract execution."""
    ...
    def _detect(self):
        ...
        """
        results = []
        if self.fortress.solc_version and self.fortress.solc_version >= "0.5.0":
            return results
