...
from fortress.utils.colors import green, yellow, red
from fortress.formatters.exceptions import FormatImpossible
from fortress.formatters.utils.patches import apply_patch, create_diff
from fortress.utils.comparable_enum import ComparableEnum
from fortress.utils.output import Output
...
class AbstractDetector(metaclass=abc.ABCMeta):
    ARGUMENT = ""  # run the detector with fortress.py --ARGUMENT
            ...
            for result in results:
                try:
                    ...
                    for file in result["patches"]:
                        original_txt = self.fortress.source_code[file].encode("utf8")
                        ...
                        diff = create_diff(self.fortress, original_txt, patched_txt, file)
                        if not diff:
    ...
    def generate_result(self, info, additional_fields=None):
        output = Output(
            ...
            markdown_root=self.fortress.markdown_root,
        )
