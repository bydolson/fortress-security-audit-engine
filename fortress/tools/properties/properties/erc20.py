...
from fortress.tools.properties.addresses.address import Addresses
from fortress.tools.properties.platforms.echidna import generate_echidna_config
from fortress.tools.properties.properties.ercs.erc20.properties.burn import ERC20_NotBurnable
from fortress.tools.properties.properties.ercs.erc20.properties.initialization import ERC20_CONFIG
from fortress.tools.properties.properties.ercs.erc20.properties.mint import ERC20_NotMintable
from fortress.tools.properties.properties.ercs.erc20.properties.mint_and_burn import (
    ERC20_NotMintableNotBurnable,
)
from fortress.tools.properties.properties.ercs.erc20.properties.transfer import (
    ERC20_Transferable,
    ERC20_Pausable,
)
from fortress.tools.properties.properties.ercs.erc20.unit_tests.truffle import generate_truffle_test
from fortress.tools.properties.properties.properties import (
    property_to_solidity,
    Property,
)
from fortress.tools.properties.solidity.generate_properties import (
    generate_solidity_properties,
    generate_test_contract,
    generate_solidity_interface,
)
from fortress.utils.colors import red, green
...
def _check_compatibility(contract):
    ...
    if not contract.is_erc20():
        errors = f"{contract} is not ERC20 compliant. Consider checking the contract with fortress-check-erc"
        return errors

    transfer = contract.get_function_from_signature("transfer(address,uint256)")

    if transfer.visibility != "public":
        errors = f"fortress-prop requires {transfer.canonical_name} to be public. Please change the visibility"
    ...
    if transfer_from.visibility != "public":
        if errors:
            errors += "\n"
        errors += f"fortress-prop requires {transfer_from.canonical_name} to be public. Please change the visibility"

    approve = contract.get_function_from_signature("approve(address,uint256)")
    if approve.visibility != "public":
        if errors:
            errors += "\n"
        errors += f"fortress-prop requires {approve.canonical_name} to be public. Please change the visibility"
