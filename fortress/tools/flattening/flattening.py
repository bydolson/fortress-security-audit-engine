...
from fortress.core.declarations import SolidityFunction, Enum
from fortress.core.declarations.contract import Contract
from fortress.core.declarations.structure import Structure
from fortress.core.solidity_types import MappingType, ArrayType
...
from fortress.slithir.operations import NewContract, TypeConversion, SolidityCall
from fortress.tools.flattening.export.export import (
    Export,
    export_as_json,
    save_to_zip,
    save_to_disk,
...
logger = logging.getLogger("Fortress-flattening")
...
class Flattening:
    ...
    def _check_abi_encoder_v2(self):
        ...
        :return:
        """
        for p in self._fortress.pragma_directives:
    ...
    def _get_source_code(
        ...
        content = self._fortress.source_code[src_mapping["filename_absolute"]].encode("utf8")
            ...
            for f in contract.functions_declared:
                ...
                if f.visibility == "external":
                    ...
                    if regex:
                        ...
                        raise FortressException(f"External keyword not found {f.name} {attributes}")
    ...
    def _pragmas(self) -> str:
        ...
        elif self._fortress.solc_version:
            ret += f"pragma solidity {self._fortress.solc_version};\n"
    ...
    def _export_most_derived(self) -> List[Export]:
        ret: List[Export] = []
        for contract in self._fortress.contracts_derived:
    ...
    def _export_all(self) -> List[Export]:
        ...
        contract_seen = set()
        contract_to_explore = list(self._fortress.contracts)
    ...
    def _export_with_import(self) -> List[Export]:
        exports: List[Export] = []
        for contract in self._fortress.contracts:
            list_contracts: List[Contract] = []  # will contain contract itself
            self._export_list_used_contracts(contract, set(), list_contracts)
    ...
    def export(  # pylint: disable=too-many-arguments,too-few-public-methods
        ...
        if target is None:
            ...
            contract = self._fortress.get_contract_from_name(target)
            if contract is None:
