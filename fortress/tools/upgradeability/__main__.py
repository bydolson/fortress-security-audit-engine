...
from fortress.utils.output import output_to_json
from fortress.tools.upgradeability.checks import all_checks
from fortress.tools.upgradeability.checks.abstract_checks import AbstractCheck
from fortress.tools.upgradeability.utils.command_line import (
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fortress Upgradeability Checks. For usage information see https://github.com/crytic/fortress/wiki/Upgradeability-Checks.",
        usage="fortress-check-upgradeability contract.sol ContractName",
    )
...
def main():
    ...
    try:
        variable1 = Fortress(v1_filename, **vars(args))
        ...
        if args.proxy_name:
            if args.proxy_filename:
                proxy = Fortress(args.proxy_filename, **vars(args))
            else:
                proxy = variable1
        ...
        if args.new_contract_name:
            if args.new_contract_filename:
                variable2 = Fortress(args.new_contract_filename, **vars(args))
            else:
                variable2 = variable1
        ...
        logger.error(str(fortress_exception))
        if args.json:
            output_to_json(args.json, str(fortress_exception), json_results)
        return
