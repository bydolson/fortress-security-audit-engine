...
def resolve_function(fortress, contract_name, function_name):
    """
...
def resolve_functions(fortress, functions):
    """
    ...
    for item in functions:
        if isinstance(item, str):
            ...
            resolved.append(resolve_function(fortress, parts[0], parts[1]))
            ...
            resolved.append(resolve_function(fortress, item[0], item[1]))
        else:
            raise ResolveFunctionException(
...
def __find_target_paths(fortress, target_function, current_path=None):
    ...
    for contract in fortress.contracts:
        for function in contract.functions_and_modifiers_declared:
            ...
            if all_target_functions.intersection(called_functions):
                path_results = __find_target_paths(fortress, function, current_path.copy())
                if path_results:
...
def find_target_paths(fortress, target_functions):
    """
    ...
    for target_function in target_functions:
        results = results.union(__find_target_paths(fortress, target_function))
