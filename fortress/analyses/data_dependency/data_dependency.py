...
from fortress.slithir.operations import Index, OperationWithLValue, InternalCall
...
def is_tainted(variable, context, only_unprotected=False, ignore_generic_taint=False):
    ...
    taints = fortress.context[KEY_INPUT]
...
def is_tainted_ssa(variable, context, only_unprotected=False, ignore_generic_taint=False):
    ...
    if isinstance(variable, Constant):
        return False
    fortress = context.fortress
    taints = fortress.context[KEY_INPUT_SSA]
    if not ignore_generic_taint:
        taints |= GENERIC_TAINT
    return variable in taints or any(
...
def compute_dependency(fortress):
    fortress.context[KEY_INPUT] = set()
    fortress.context[KEY_INPUT_SSA] = set()

    for contract in fortress.contracts:
        compute_dependency_contract(contract, fortress)


def compute_dependency_contract(contract, fortress):
    ...
    for function in contract.functions + contract.modifiers:
        ...
        if function.visibility in ["public", "external"]:
            [fortress.context[KEY_INPUT].add(p) for p in function.parameters]
            [fortress.context[KEY_INPUT_SSA].add(p) for p in function.parameters_ssa]
